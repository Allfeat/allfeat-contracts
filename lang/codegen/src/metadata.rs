...
impl Drop for LockedTrait {
    fn drop(&mut self) {
        ...
        self.file
            ...
            .expect("Can't set cursor position");
        if let Some(trait_definition) = &self.trait_definition {
            serde_json::to_writer(
                &self.file,
                &trait_definition.0.to_token_stream().to_string(),
            )
...
fn get_locked_file(name: String) -> File {
    ...
    let target: String = target
        ...
        .skip(INK_PREFIX.len())
        .take(
            target.find(SUFFIX).expect("Unable to find debug/deps") - INK_PREFIX.len()
                + SUFFIX.len(),
        )
        .collect();

    let target_dir =
        Utf8PathBuf::from_str(target.as_str()).expect("Can't generate Path from target");
